...
$(document).ready(function () {
	...
	function displayStaticContent() {
		...
		$(".container").find(".draggableitemWraper").each(function(index,el){
			$(el).css({"position":"absolute","top":top+"px","left":left+"%"});
			$(el).find('.draggableitem').prop('disabled',true).attr('aria-hidden',"true");
			var width= $(el).width();
			// left=left+width+10;
			left=left+25;
			if((index+1)%4==0)
			...
			if(index>=totalItemLen-4){
				// $(el).find('.draggableitem').removeAttr('disabled');
				$(el).find('.draggableitem').removeAttr('disabled aria-hidden');
			}
			});
	...
	function selectCategory() {
		...
		if (id) {
			...
			if(index>=4){
				...
				$(".draggableitemCnt_"+(index-4)).find(".draggableitem").removeAttr('disabled aria-hidden');
			...
			if (detectMob()) {
				...
				$(".draggableitemCnt_" + index).find(".draggableitem").removeAttr('disabled aria-hidden');
			}
			...
			if (detectMob()) {
				...
				// $(".container .draggableitem").last().removeAttr('disabled');
				$(".container .draggableitem").last().removeAttr('disabled aria-hidden');
			}
			...
			$('.category').each(function (index, el) {
				...
				$(el).find(".categoryDroppableCnt").children().each(function (ind, ell) {
					$(ell).find(".draggableitem").removeClass("correct incorrect");
	...
	function resetCategory(objThis) {
		...
		if (detectMob()) {
			...
			var totalItemLen = arrAllDraggableitem.length;
		$(".container .draggableitem").prop('disabled', true).attr('aria-hidden','true');
		$(".container").find(".draggableitemWraper").each(function(index,el){
			if(index>=totalItemLen-4){
				// $(el).find('.draggableitem').removeAttr('disabled');
				$(el).find('.draggableitem').removeAttr('disabled aria-hidden');
